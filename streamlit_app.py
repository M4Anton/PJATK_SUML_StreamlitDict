...
option = st.selectbox(
    ...
    [
        ...
        "Tłumaczenie tesktu z języka angielskiego na niemiecki",
    ],
)
...
if option == "Wydźwięk emocjonalny tekstu (eng)":
    ...
    if text:
        ...
        st.write(answer)
elif option == "Tłumaczenie tesktu z języka angielskiego na niemiecki":
    st.text('''Aby przetłumaczyć tekst z angielskiego na niemiecki wpisz szukaną frazę
w elemencie poniżej, a następnie naciśnij CTRL + ENTER, aby wyszukać tłumaczenie''')	
    text = st.text_area(label="Wpisz tekst do przetłumaczenia")
    if text:
        with st.spinner("Ładuję..."):
            translator = pipeline("translation_en_to_de")
            translation = translator(text, max_length=250)
        st.success("Załadowano!")
        st.text("Twój tekst po niemiecku:")
        st.text(translation[0].get("translation_text"))
